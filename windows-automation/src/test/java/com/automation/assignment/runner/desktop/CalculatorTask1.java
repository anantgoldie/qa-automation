...
public class CalculatorTask1 {
	...
	public void add() {
        ...
        Assert.assertEquals(getResults().getText(), "10");
        
        Utility.getCalculatorScreenshot(screenShotLocation + Utility.getScreenshotFileName(++stepCount, methodName));
	}
	
	public void subtract() {
		Calculator.session.findElementByName(Calculator.subtract).click();
		Calculator.session.findElementByName(Calculator.three).click();
		Calculator.session.findElementByName(Calculator.equals).click();
        Assert.assertEquals(getResults().getText(), "7");
